...
export default class SidebarQueries extends Component {
      ...
      return this.props.queries.map((val, index) => {
        return (
          <li key={index}>
            <Link
              to={`/query/${this.props
                .namespace}/${val.id}/${val.lastRevisionNum}`}
            >
              {val.id}
            </Link>
          </li>
  ...
  render() {
    const shouldCollapse =
      this.props.namespace === this.props.collapsedNamespace &&
      !this.props.loadingQueries;
    return (
      <ul className="queries">
        <Collapse in={shouldCollapse}>
          <div>{shouldCollapse ? this.renderQueries() : null}</div>
        </Collapse>
      </ul>
    );
  }
